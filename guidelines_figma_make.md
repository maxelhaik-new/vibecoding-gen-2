...
1. **Phase: Découpage (`DECOUPE`)** :
   À partir d'un sujet, d'un plan ou de notes, proposer un découpage slide par slide (Titre de la slide, concept, objectif, choix recommandé du template en se référant à @templates_charter.md).
   
2. **Phase: Écriture (`ECRIS`)** :
   Pour chaque slide, attribuer le template V2 le plus approprié et rédiger les textes correspondants. Présenter uniquement en **format lecture humaine** — aucun JSON ni image ne doit être produit à cette étape. Attendre la validation de l'utilisateur.
   *   **Règles d'écriture** : Consulter systématiquement le fichier @brand_voice.md pour appliquer le ton Wemodo (bannir "tu"/"vous", infinitifs, vocabulaire interdit).
   *   **Pas de numérotation** : Ne jamais ajouter de chiffres ou numéros (ex: "1.", "2.") au début des titres de blocs ou d'objectifs, car ils sont déjà intégrés dans les composants Figma.
   *   **Gestion des longueurs** :
       ...
       *   *Cibles* : Les valeurs de `target_lenght` servent d'idéal esthétique, mais il est tout à fait acceptable d'être en dessous de cette cible tant que le texte est complet, correct et respecte le `min_lenght`.
   *   **Icônes (`Picto 1`, `Picto 2`, etc.)** : Renseigner pour chaque clé Picto présente dans le template un identifiant d'icône au format `mdi:nom-icone` issu de @icon_mapping.md. Ne pas laisser ces champs vides.
...
3. **Phase: Génération (`GÉNÈRE`)** :
   ...
   1. **Génération automatique des images** : Exécuter le script `scripts/generate_nano_banana.py` pour chaque slide utilisant un template contenant une image (ex: `VIBECODING - 3 BLOCS - PHOTO`, `VIBECODING - 3 BLOCS - PHOTO - ALT`, `VIBECODING - IMAGE`, `VIBECODING - USE CASE`, `VIBECODING - FOCUS OUTIL`, `PODIUM`, `CHIFFRES - PHOTO`), avec le concept/sujet (soit validé à l'étape ECRIS, soit déduit du contexte de la slide si absent).
   2. **Production du JSON** : Produire le JSON final prêt à coller dans le plugin Figma. Inclure les URLs `http://localhost:8080/assets/[fichier_genere.png]` dans les champs `"image"`. S'assurer que tous les calques du template (y compris les clés `Picto X` et la clé `"image"`) sont présents.
...
La structure des templates, leurs statuts de validation (validé / en attente) et leurs contraintes de caractères se trouvent dans le fichier suivant :
...
```json
{
  "lessonTitle": "Titre Global de la Leçon",
  "slides": [
    {
...
2. **Adaptation Automatique du Ratio (Sur-Mesure)** :
   - Avant de lancer la génération pour un template contenant une illustration (ex: `VIBECODING - 3 BLOCS - PHOTO` ou `CHIFFRES - PHOTO`), l'agent doit récupérer les dimensions exactes du composant d'image cible (calque nommé `Image Fond`, `photo`, ou similaire).
