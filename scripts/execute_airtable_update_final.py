...
def main():
    print("==================================================================")
    print("EXÉCUTION DE LA MISE À JOUR AIRTABLE MODULE 5 (CHAPITRES 2 À 7)")
    print("==================================================================")

    all_records = fetch_all_m5_records()
    ...
    for c in chapters:
        ...
        # Get exact sample fields for Module, Chapitre, PHASE, État
        ...
        if existing_ids:
            ...
            with urllib.request.urlopen(req_patch) as resp:
                print(f"  ✅ {len(existing_ids)} leçons existantes réorientées vers N° 10.")

        # 2. Filter existing lesson titles in python
        chap_records = [r for r in all_records if r["fields"].get("Chapitre") == chap_name]
        existing_titles = {r["fields"].get("Nom de la leçon") for r in chap_records}

        # Create new lessons one by one safely
        created_count = 0
        for nl in new_lessons:
            ...
            req_post = urllib.request.Request(patch_url, data=json.dumps(single_payload).encode('utf-8'), headers=headers, method="POST")
            try:
                with urllib.request.urlopen(req_post) as resp:
                    res = json.loads(resp.read().decode('utf-8'))
                    rec_created = res["records"][0]
                    created_count += 1
                    # Add to existing titles set so duplicates won't happen
    ...
    print("\n==================================================================")
    print("🎉 MISE À JOUR GLOBAL AIRTABLE DU MODULE 5 TERMINÉE AVEC SUCCÈS !")
    print("==================================================================")
