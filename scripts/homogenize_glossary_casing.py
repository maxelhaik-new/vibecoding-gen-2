...
# Normalize casing rules according to standard French glossary rules:
# Capitalize acronyms (API, CSS, DOM, HTML, IDE, JSON, LLM, RAG, SQL, UI, CLI)
# Preserve proper names/brands (Antigravity, Bolt.new, Claude 3.5 Sonnet, Cursor, Expo, Express.js, Git, GitHub, JavaScript, Lovable, Node.js, React, React Native, Tailwind CSS, v0 (by Vercel))
# Sentence case for generic terms/methods/concepts
...
# Sort alphabetically by term (case insensitive, ignoring leading dot)
sorted_cased_terms = sorted(casing_map, key=lambda x: x["mot"].lstrip(".").lower())

md_lines = []
md_lines.append("# 📚 Glossaire Officiel de la Formation Vibe Coding")
md_lines.append("\nCe glossaire regroupe le vocabulaire technique des **Modules 1 à 4**, trié par **ordre alphabétique** avec une **casse classique de glossaire** (majuscules réservées aux acronymes, noms propres et marques).\n")
md_lines.append("| Mot / Concept | Statut | Type de terme | Définition | Référence de la leçon |")
md_lines.append("| :--- | :--- | :--- | :--- | :--- |")

for item in sorted_cased_terms:
    mot = item["mot"]
...
print(f"✅ Casse homogénéisée avec succès dans {target_path} ({len(sorted_cased_terms)} termes).")
